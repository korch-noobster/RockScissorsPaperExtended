...
namespace RockPaperScissors
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            class RPSGame
        {
            private string aiWeapon;
             public string ByteDecode(byte[] ms)
            {
                string hash = String.Empty;
                foreach(byte theByte in ms)
                {
            ...
            public  string GenerateNewHMAC(string password, string message)
            {
                 // HMACSHA256 hmc = new HMACSHA256(password);
                HMACSHA256 hmc = new HMACSHA256(Encoding.ASCII.GetBytes(password));
                byte[] hash = hmc.ComputeHash(Encoding.ASCII.GetBytes(message));
            ...
            public void NextAI()    
            {
                aiWeapon = _rpsAI.NextMove(); // Gets the AI opponent's next move.
